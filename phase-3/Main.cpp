...
int main() {
    ...
    cout << dataset.get_collisions().get_size() << " records loaded in ";
    cout << chrono::duration<double>(end_time_load - start_time_load).count() << " seconds." << endl;

    if (dataset.get_collisions().get_size() == 0) {
    ...
    vector<int> collisions_in_zipcode = dataset.search_by_zip_code(zip_code);
    ...
    vector<int> collisions_with_injuries = dataset.search_by_injury_ranges(inj_low, inj_high);
    ...
    vector<int> collisions_on_latitude = dataset.search_by_latitude(lat);
    ...
    vector<int> collisions_in_date_range = dataset.search_by_date_range(start_date, end_date);
    ...
    vector<int> collisions_in_borough = dataset.search_by_borough(borough);
